...
function App() {
  ...
  const getData = async()=>{
    try{
      const getDataRef = await axios.get("https://fakestoreapi.com/products");
      console.log("取得資料成功",getDataRef.data);
      setDate(getDataRef.data);
    }catch(error){
      console.log("取得資料失敗",error.data);
    }
  }
